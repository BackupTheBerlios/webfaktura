...
class kunden extends page{
	...
	function rechnung_pdf($id){
		...
		$pdf->Cell(100,5,"Gesamt: ".number_format($betrag->Gesamt,2,",",".").EURO, 0, 1, 'R');
		$pdf->Cell(100,5,"MwSt (".number_format($betrag->satz)."%): ".number_format($betrag->MWST,2,",",".").EURO, 0, 1, 'R');
		$pdf->Ln();
		$pdf->Cell(100,5,"Rechnungsbetrag: ".number_format($betrag->Gesamt+$betrag->MWST,2,",",".").EURO, 0, 1, 'R');
		$pdf->Ln();
